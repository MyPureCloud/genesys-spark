...
async function clickDropdownButton(page: E2EPage): Promise<void> {
  return await page.evaluate(() => {
    const element = document.querySelector('gux-button-multi-legacy');
...
describe('gux-button-multi-legacy', () => {
  const html = `
  <gux-button-multi-legacy lang="en" text="Primary" accent="primary">
  ...
  it('renders', async () => {
    ...
    const element = await page.find('gux-button-multi-legacy');
