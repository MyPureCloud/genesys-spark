...
createComponentStencilFiles = (componentName, componentType, dir) => {
  const tsxTpl = fs.readFileSync(
    path.resolve(tplPath, 'component.tsx.hbs'),
    'utf8'
  );
  const lessTpl = fs.readFileSync(
    path.resolve(tplPath, 'component.less.hbs'),
    'utf8'
  );
  ...
  const specTpl = fs.readFileSync(
    path.resolve(tplPath, 'component.spec.ts.hbs'),
    'utf8'
  );
  const e2eTpl = fs.readFileSync(
    path.resolve(tplPath, 'component.e2e.ts.hbs'),
    'utf8'
  );
  const data = {
  ...
  fs.writeFileSync(
    path.resolve(dir, componentName + '/' + componentName + '.tsx'),
    Handlebars.compile(tsxTpl)(data)
  );
  fs.writeFileSync(
    path.resolve(dir, componentName + '/' + componentName + '.less'),
    Handlebars.compile(lessTpl)(data)
  );
  fs.writeFileSync(
    path.resolve(dir, componentName + '/tests/' + componentName + '.spec.ts'),
    Handlebars.compile(specTpl)(data)
  );
  fs.writeFileSync(
    path.resolve(dir, componentName + '/tests/' + componentName + '.e2e.ts'),
    Handlebars.compile(e2eTpl)(data)
  );
  fs.writeFileSync(
    path.resolve(dir, componentName + '/readme.md'),
    ...
    },
    {
      ...
      validate: componentName => {
        ...
        return fs.existsSync(target) ||
          fs.existsSync(target2) ||
          fs.existsSync(target3)
