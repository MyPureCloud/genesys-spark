 ...
 * 3. Build component relationship graphs based on co-usage and hierarchy
 * 4. Generate usage documentation and reference structures
 * 5. Enhance JSON docs with relatedComponents, parentComponent, and usage references
 * 6. Create comprehensive component metadata for the MCP server
 */
...
/**
 * Find the most relevant example for a component based on proximity to the component location
 */
function findMostRelevantExample(componentId, examplePaths, jsonDocs) {
    const result = scoreAndSortExamples(componentId, examplePaths, jsonDocs);
    return result.primaryExample;
}
...
function createEnhancedDocStructure(originalDocs) {
    ...
    // Handle sub-components with references to parent examples
...
/**
 * Add relationship data to each component
 */
/**
...
function addParentComponentInfo(components) {
    for (const component of components) {
        const componentId = component.tag;
        const parentId = findParentComponent(componentId, components);
        component.parentComponent = parentId;
    }
}
...
/**
 * Find parent component based on folder structure from filePath
 */
function findParentComponent(componentId, allComponents) {
    ...
    for (const component of allComponents) {
        ...
        // Check if the candidate's directory is a parent of current directory
