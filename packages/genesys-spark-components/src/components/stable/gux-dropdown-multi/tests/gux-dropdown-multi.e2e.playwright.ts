...
test.describe('gux-dropdown-multi', () => {
  ...
  test.describe('filter', () => {
    ...
    test.describe('with the create option set', () => {
      ...
      // test('does not provide a create option if there is an exact match', async ({ page }) => {
      //   await setContent(page, creatableDropdown);
      //   const component = page.locator('gux-dropdown-multi');
      ...
      test('updates dropdown multi and listbox value when slot changes', async ({
        ...
        // const listbox = component.locator('gux-listbox-multi');
        const listbox = page.locator('gux-listbox-multi');
        let dropdownValue = await component.getAttribute('value');
        let listboxValue = await listbox.getAttribute('value');
        let selectedItems = component.locator('gux-option-multi.gux-selected');
        // let selectedItems = await page.findAll('.gux-selected');

        expect(await selectedItems.count()).toBe(0);
        expect(dropdownValue).toBeNull();
        expect(listboxValue).toBeNull();
        ...
        expect(await selectedItems.count()).toBe(1);
        dropdownValue = component;
        listboxValue = listbox;
        await expect(dropdownValue).toHaveAttribute('value', 'newoption');
        await expect(listboxValue).toHaveAttribute('value', 'newoption');
      });
    });
  });
});
