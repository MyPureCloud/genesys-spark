...
export class GuxTabsLegacy {
  ...
  watchHandler(newValue: string) {
    const tabs: HTMLGuxTabLegacyElement[] = Array.from(
      this.root.querySelectorAll('gux-tab-legacy')
    );
  ...
  internaltabselectedHandler(e: CustomEvent) {
    whenEventIsFrom('gux-tab-legacy', e, elem => {
      const tab = elem as HTMLGuxTabLegacyElement;
      if (!tab.active) {
      ...
      readTask(() => {
        if (this.value) {
          const activeTab: any = this.root.querySelector(
            `gux-tab-legacy[tab-id='${this.value}']`
          );
          if (activeTab) {
