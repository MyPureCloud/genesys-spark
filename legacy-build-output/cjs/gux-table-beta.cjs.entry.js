...
const GuxTable = class {
  constructor(hostRef) {
    ...
    this.slotObserver = new MutationObserver(() => index.forceUpdate(this));
  ...
  render() {
    return (index.h(index.Host, { "gs-table-id": this.tableId, "gs-v-scroll": this.isVerticalScroll, "gs-h-scroll": this.isHorizontalScroll, "gs-obj-table": this.objectTable, "gs-compact": this.compact }, index.h("div", { class: "gux-table" }, index.h("div", { tabindex: this.isVerticalScroll ? '0' : '-1', id: this.tableId, class: this.tableContainerClasses }, index.h("slot", { name: "data" })), this.isHorizontalScroll && (index.h("gux-button-slot-beta", { accent: "secondary" }, index.h("button", { class: "gux-table-scroll-left", disabled: this.isScrolledToFirstCell, onClick: !this.isScrolledToFirstCell && this.previousColumn.bind(this) }, index.h("gux-icon", { "icon-name": "chevron-left", "screenreader-text": this.i18n('scrollLeft') })))), this.isHorizontalScroll && (index.h("gux-button-slot-beta", { accent: "secondary", style: { marginRight: `${this.tableScrollbarConstant}px` } }, index.h("button", { class: "gux-table-scroll-right", disabled: this.isScrolledToLastCell, style: { marginRight: `${this.tableScrollbarConstant}px` }, onClick: !this.isScrolledToLastCell && this.nextColumn.bind(this) }, index.h("gux-icon", { "icon-name": "chevron-right", "screenreader-text": this.i18n('scrollRight') })))), this.isTableEmpty && (index.h("div", { class: "gux-empty-table" }, this.emptyMessage || this.i18n('emptyMessage'))))));
  }
  get root() { return index.getElement(this); }
};
GuxTable.style = guxTableCss;

exports.gux_table_beta = GuxTable;
