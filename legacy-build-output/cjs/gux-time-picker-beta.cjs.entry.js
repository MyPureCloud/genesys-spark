...
const index$1 = require('./index-d3bc59d7.js');
...
const index = require('./index-c4441830.js');
...
function getLocaleClockType(root) {
  const locale = index.getDesiredLocale(root);
...
const GuxTimePickerBeta = class {
  ...
  async componentWillLoad() {
    ...
    this.i18n = await index.buildI18nForComponent(this.root, translationResources);
  ...
  renderNumberInput() {
    return (index$1.h("div", { class: "gux-input-time-container" }, index$1.h("input", { class: "gux-input-time-hours", type: "text", disabled: this.disabled, value: getHourDisplayValue(this.value, this.clockType), onKeyDown: e => this.onHourKeyDown(e), "aria-label": this.i18n('hoursInput'), pattern: getHoursPattern(this.clockType), ref: el => (this.hourInputElement = el) }), index$1.h("span", { class: "gux-time-separator" }, this.i18n('time-separator')), index$1.h("input", { class: "gux-input-time-minutes", type: "text", disabled: this.disabled, value: getMinuteDisplayValue(this.value), onKeyDown: e => this.onMinuteKeyDown(e), "aria-label": this.i18n('minutesInput'), pattern: getMinutesPattern(), ref: el => (this.minuteInputElement = el) })));
  }
  renderAmPmSelector() {
    if (this.clockType === '12h') {
      return (index$1.h("button", { class: "gux-input-time-am-pm-selector", type: "button", disabled: this.disabled, "aria-label": this.i18n('toggleAmPM', { amOrPm: this.getAmPmString() }), onClick: (e) => this.toggleAmPm(e), onKeyDown: (e) => this.onAmPmButtonKeyDown(e) }, index$1.h("div", { class: {
          'gux-meridiem': true,
          'gux-visible': isAm(this.value)
        } }, this.i18n('am')), index$1.h("div", { class: {
          'gux-meridiem': true,
          'gux-visible': !isAm(this.value)
        } }, this.i18n('pm'))));
    }
  ...
  renderClockButton() {
    return (index$1.h("button", { class: {
      ...
      }, type: "button", disabled: this.disabled, "aria-label": this.i18n('clockButton'), "aria-expanded": this.expanded.toString(), onClick: this.toggleDropdown.bind(this), ref: el => (this.clockButton = el) }, index$1.h("gux-icon", { decorative: true, "icon-name": "clock-outline" })));
  }
  renderTimeListItems() {
    return getTimeDisplayValues(this.interval, this.clockType).map(displayValue => {
      const value = getValue(displayValue, this.clockType, isAm(this.value));
      return (index$1.h("gux-list-item", { id: this.valueToId(value), onClick: () => this.handleClickDropdownValue(displayValue) }, displayValue));
    });
  }
  renderTarget() {
    return (index$1.h("div", { class: "gux-input-time", slot: "target" }, this.renderNumberInput(), this.renderAmPmSelector(), this.renderClockButton()));
  }
  renderPopup() {
    return (index$1.h("div", { slot: "popup", class: "gux-list-container", onKeyDown: (e) => this.onListKeyDown(e) }, index$1.h("gux-list", { ref: el => (this.listElement = el) }, this.renderTimeListItems())));
  }
  render() {
    return (index$1.h("gux-popup-beta", { class: {
  ...
  get root() { return index$1.getElement(this); }
};
__decorate([
...
exports.gux_time_picker_beta = GuxTimePickerBeta;
