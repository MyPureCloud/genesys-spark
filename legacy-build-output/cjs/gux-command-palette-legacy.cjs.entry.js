...
const search$1 = require('./search-a2ed7f26.js');
...
const GuxCommandPaletteLegacy = class {
  ...
  async componentWillLoad() {
    ...
    this.i18n = await index$1.buildI18nForComponent(this.root, paletteResources);
  }
  render() {
    return (index.h("div", { class: `gux-command-palette ${this.visible ? '' : 'gux-hidden'}`, role: "dialog", onKeyDown: e => this.onKeyDown(e), "aria-label": this.i18n('title') }, index.h("label", { htmlFor: "gux-command-palette-search", class: "gux-command-palette-search-label" }, this.i18n('search')), index.h("gux-input-search", null, index.h("input", { id: "gux-command-palette-search", type: "search", onInput: (event) => {
  ...
  renderLists() {
    ...
    if (this.filterValue && filteredItems.length) {
      ...
      const filterList = [
        index.h("gux-list-legacy", { tabindex: "-1", highlight: this.filterValue }, filteredItems)
      ];
      if (filterExceeded) {
        filterList.unshift(index.h("div", { class: "gux-limit" }, this.i18n('limited')));
      }
    ...
    if (!lists.length) {
      return (index.h("gux-list-legacy", { tabindex: "-1" }, this.transformCommands(sortActions(allItems))));
    }
    return lists;
  }
  /**
  ...
  filterItems(items) {
    return this.transformCommands(sortActions(items.filter((item) => {
      return search$1.matchesFuzzy(this.filterValue, `${item.text} + ${item.details}`);
    })), '');
  }
  createShortcutItem(command) {
    return (index.h("gux-list-item-legacy", { value: command.text, onPress: this.handlePress(command), class: (command.details && 'gux-has-details') || '' }, index.h("div", null, index.h("gux-text-highlight", { text: command.text, strategy: "fuzzy" }), index.h("span", { class: "gux-shortcut" }, command.shortcut)), command.details && (index.h("gux-text-highlight", { class: "gux-details", text: command.details, strategy: "fuzzy" }))));
  }
  createStandardItem(command) {
    return (index.h("gux-list-item-legacy", { value: command.text, onPress: this.handlePress(command), class: (command.details && 'gux-has-details') || '' }, index.h("gux-text-highlight", { text: command.text, strategy: "fuzzy" }), command.details && (index.h("gux-text-highlight", { class: "gux-details", text: command.details, strategy: "fuzzy" }))));
  }
  transformCommands(commands, header) {
    const retVal = [];
    if (header) {
      retVal.push(index.h("strong", null, header));
    }
  ...
  createList(items, filter, header) {
    return (index.h("gux-list-legacy", { class: "gux-command-palette-list", highlight: filter, tabindex: "-1" }, this.transformCommands(sortActions(items), header)));
  }
  onKeyDown(event) {
    switch (event.key) {
      case 'ArrowUp':
...
exports.gux_command_palette_legacy = GuxCommandPaletteLegacy;
