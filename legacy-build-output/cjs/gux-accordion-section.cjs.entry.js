...
const GuxAccordionSection = class {
  ...
  handleSlotChange(slotname) {
    ...
    if (!slot || !/^H[1-6]$/.test(slot.nodeName)) {
      logError.logError('gux-accordion-section', `For accessibility reasons the ${slotname} slot should be filled with a HTML heading tag (h1 - h6).`);
    }
  }
  componentWillLoad() {
  ...
  render() {
    return (index.h("section", { class: { 'gux-disabled': this.disabled } }, index.h("button", { class: {
        'gux-header': true,
        'gux-reverse-headings': this.reverseHeadings
      }, "aria-expanded": this.open.toString(), "aria-controls": this.sectionId, disabled: this.disabled, onClick: this.toggle.bind(this) }, this.hasIconSlot && index.h("slot", { name: "icon" }), index.h("div", { class: {
        'gux-header-text': true,
        'gux-arrow-position-beside': this.isArrowPositionedBesideText()
      } }, index.h("slot", { onSlotchange: () => this.handleSlotChange('header'), name: "header" }), index.h("slot", { onSlotchange: () => this.handleSlotChange('subheader'), name: "subheader" })), index.h("div", { class: {
        'gux-header-icon': true,
        'gux-expanded': this.open,
        'gux-arrow-position-before-text': this.isArrowPositionBeforeText()
      } }, index.h("gux-icon", { decorative: true, "icon-name": "chevron-small-down" }))), index.h("div", { id: this.sectionId, class: {
        'gux-content': true,
        'gux-expanded': this.open,
...
exports.gux_accordion_section = GuxAccordionSection;
