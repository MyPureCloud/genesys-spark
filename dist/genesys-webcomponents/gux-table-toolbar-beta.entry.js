import { r as registerInstance, k as readTask, h, j as Host, g as getElement } from './index-f583fcde.js';
...
function collapseActionsAll(actionsFilterContextual, actionsPerm, actionPrimary) {
  [].concat(actionsFilterContextual, actionsPerm, actionPrimary).forEach(action => {
    if (action != null) {
      action.iconOnly = true;
    }
  });
}
function expandActionsAll(actionsFilterContextual, actionsPerm, actionPrimary) {
  [].concat(actionsPerm, actionPrimary, actionsFilterContextual).forEach(action => {
    if (action != null) {
      action.iconOnly = false;
    }
  });
...
const GuxTableToolbar = class {
  ...
  renderFullLayout() {
    this.displayedLayout = 'full';
    expandActionsAll(this.allFilterContextual, this.permanentActions, this.primaryAction);
  }
  renderIconOnlyLayoutScaleDown(controlWidth) {
    this.displayedLayout = 'iconOnly';
    collapseActionsAll(this.allFilterContextual, this.permanentActions, this.primaryAction);
